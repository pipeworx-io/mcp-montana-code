 ...
 *
 * The path is the citation exploded and zero-padded to four digits each, with
 * one part that is NOT written in the citation: the PART number, which is the
 * leading digit of the section.
 *
 *   MCA 45-5-102  ->  title 45, chapter 5, part 1, section 02
 *   /title_0450/chapter_0050/part_0010/section_0020/0450-0050-0010-0020.html
 *
 * So "102" is read as part 1 + section 02. Nobody writes the part in a
 * citation and nobody would guess the padding, which is the whole reason this
 * is a resolver rather than a link.
 *
 * Honest upstream: an unknown section returns 404.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Montana Code Annotated');
}

const BASE = 'https://archive.legmt.gov/bills/mca';
const UA = 'pipeworx-mcp-montana-code/1.0 (+https://pipeworx.io)';

const pad = (s: string) => `${s.padStart(3, '0')}0`;

const tools: McpToolExport['tools'] = [{
  name: 'mt_code_section',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "45-5-102" (deliberate homicide) or "61-8-401" (DUI). Title-chapter-section.' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'mt_code_section') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '').replace(/^MCA/i, '');
  const m = /^(\d+)-(\d+)-(\d)(\d{2})$/.exec(raw);
  if (!m) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Montana Code citation this can resolve.`,
      hint: 'MCA sections are title-chapter-section with a three-digit section, e.g. "45-5-102" (deliberate homicide) or "61-8-401" (DUI).' };
  }
  const [, title, chapter, part, sec] = m;
  const t = pad(title), c = pad(chapter), p = pad(part), s = pad(sec);
  const url = `${BASE}/title_${t}/chapter_${c}/part_${p}/section_${s}/${t}-${c}-${p}-${s}.html`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'section_not_found', section: raw, title, chapter,
      message: `No section ${raw} in the Montana Code Annotated (HTTP 404).`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  if (!res.ok) throw new Error(`Montana Legislature returned HTTP ${res.status} for ${raw}`);
  const full = statuteText(await readStatuteResponse(res));
  // The page title repeats the citation before the site nav ("45-5-102.
  // Deliberate homicide, MCA / Toggle navigation / Contents ..."), so anchor
  // on the LAST occurrence of the heading — the first is the <title>, the
  // statute follows the menu.
  const esc = raw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const re = new RegExp(`${esc}\\.\\s`, 'g');
  let at = -1;
  for (let mm = re.exec(full); mm; mm = re.exec(full)) at = mm.index;
  const text = at === -1 ? full : full.slice(at).trim();
  const heading = (text.match(/\d+-\d+-\d+\.\s*([^.]{2,120})\./) ?? [])[1] ?? null;
  return {
    citation: `MCA § ${raw}`, section: raw, title, chapter, part,
    heading: heading ? heading.trim() : null,
    text, text_chars: text.length, url,
    source: 'Montana Legislature (archive.legmt.gov), keyless',
    attribution: 'Montana statutes are public record.',
  };
}
